    ...
    // ------------------------------------------
    const titanStats = [];
    const unsuitableList = ["강인함 1", "강인함 2", "자연의 포옹", "승리의 함성", "부족의 축복 1", "부족의 축복 2", "파괴자 1", "파괴자 2", "파괴자 3", "지진"];
    const vampExclusion = ["고독한 분노", "협동 공격", "마지막 선물", "보스 슬레이어"];
    ...
    function loadUserConfig() {
      ...
      document.querySelector('#timeDropdown .selected-value').textContent = `${tLimit}분`;
      // 룬 슬롯 복구
      if (config.selectedRunes) {
        ...
        selectedRunes.forEach((rune, idx) => {
          ...
          if (rune && rune.name && RUNES_DATA[rune.name]) {
            ...
            slot.innerHTML = `<span style="font-size:10px; color:#aaa;">슬롯 ${idx+1}</span>`;
          }
        });
      }
      calcFinal(); // 화면 스탯 갱신
    ...
    window.onload = () => {
      ...
      for (let m = 10; m <= 120; m += 10) {
        ...
        li.textContent = `${m}분`;
        li.onclick = () => {
          document.querySelector('#timeDropdown .selected-value').textContent = `${m}분`;
      ...
      for (let i = 0; i < 5; i++) {
        ...
        div.className = 'slot';
        div.id = `slot-${i}`;
        div.innerHTML = `<span style="font-size:10px; color:#aaa;">슬롯 ${i+1}</span>`;
...
function applyRuneToSlot() {
      ...
      selectedRunes.forEach((rune, idx) => {
        if (rune && rune.name === tempName) {
          ...
          otherSlot.innerHTML = `<span style="font-size:10px; color:#aaa;">슬롯 ${idx+1}</span>`;
        }
      });

      selectedRunes[activeSlotIdx] = { name: tempName, lv: lv };
      slot.innerHTML = `<img src="${getImgUrl(RUNES_DATA[tempName].imgId)}" class="slot-img"><div class="slot-lv-tag">Lv.${lv}</div>`;
    ...
    function removeRuneFromSlot() {
      ...
      // 안전하게 비우고 기본 텍스트 삽입
      slot.textContent = '';
      slot.innerHTML = `<span style="font-size:10px; color:#aaa;">슬롯 ${activeSlotIdx+1}</span>`;
      document.getElementById('runePicker').style.display = 'none';
      calcFinal();
      saveUserConfig();
    }
    ...
    function getBattleStats() {
      ...
      selectedRunes.forEach(r => {
        ...
        // 1. 기본 공격력/체력 조건부 합산
        let active = (r.name === "협동 공격" ? count >= 5 : (r.name === "고독한 분노" ? count === 1 : true));
        if (active) {
    ...
    function calcFinal() {
      ...
      if (isDefenseExpected) {
        ...
        resRedEl.innerHTML = `${Math.floor(totalExp).toLocaleString()} <span style="font-size:0.75em; opacity:0.8;">(1회 피격당 기대값)</span>`;
      } else {
      ...
      if (isVampExpected) {
        resVampEl.innerHTML = `${Math.floor(s.vampExpected).toLocaleString()} <span style="font-size:0.75em; opacity:0.8;">(1회 피격당 기대값)</span>`;
      } else {
        resVampEl.innerText = s.vProb > 0 ? `${Math.floor(s.vampAmount).toLocaleString()} (${s.vProb}% 확률)` : "0";
      }
      saveUserConfig();
    }
    // 모드 전환용 함수
      ...
      const runBatch = () => {
        ...
        for (let i = completed; i < end; i++) {
          ...
          for (let t = 1; t <= limitSec; t++) {
            ...
            if (isLogEnabled && i === 0) { // 로그가 켜져 있고, 첫 번째 반복일 때만 기록
              detailedLogs.push({
                "시간": t + "초",
